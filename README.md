...
- [x] HttpSession
  ...
  - [x] void invalidate()
    - 세션을 삭제한다.
- [x] 서버 최초 접근 시 세션을 생성한다.
  - [x] 클라이언트 요청 헤더에 세션 쿠키가 없으면 최초 접근이라고 판단한다.
  - [x] 클라이언트 요청 헤더에 세션 쿠키가 있지만 서버 내부의 쿠키의 값에 해당하는 세션이 없으면 최초 접근이라고 판단한다.
  - [x] 생성한 세션은 세션 Context에 저장한다.
- [x] 클라이언트 요청에 해당하는 세션이 서버에 존재하는 경우(최초 생성된 경우 포함) 세션의 아이디를 쿠키에 추가하여 응답한다. 
- [x] 로그인 시 세션에 로그인 정보를 저장한다.
