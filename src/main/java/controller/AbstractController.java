...
public abstract class AbstractController implements Controller {
    @Override
    public void service(HttpRequest request, HttpResponse response) throws ClassNotFoundException, InvocationTargetException, IllegalAccessException, NoSuchMethodException {
        HttpMethod httpMethod = request.getMethod();
        Class<?> cls = Class.forName(this.getClass().getName());
        Method[] declaredMethods = cls.getDeclaredMethods();
        Method targetMethod = Arrays.stream(declaredMethods).filter(method -> isSameMethod(method.getName(), httpMethod.name()))
                .findFirst()
